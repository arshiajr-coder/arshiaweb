...
const optionsArray = ["brightness", "saturation", "inversion", "blur"];
...
filterOptions.forEach((item) => {
  item.addEventListener("click", () => {
    ...
    if (optionsArray.indexOf(item.id) !== -1) {
      if (item.id === "inversion") {
        filterSlider.max = "100";
      }
      if (item.id === "blur") {
        filterSlider.max = "50";
        sliderPrecent.textContent = `${blurW}px`;
        filterSlider.value = `${blurW}`;
      } else {
...
const updateSlider = () => {
  ...
  switch (document.querySelector(".option-active").id) {
    ...
    case "inversion":
      ...
      break;
    case "blur":
      blurW = filterSlider.value;
      sliderPrecent.textContent = filterSlider.value + "px";
      break;
  }
  previewImg.style.filter = `brightness(${brightness}%) saturate(${saturation}%) invert(${inversion}%) blur(${blurW}px)`;
};
...
const downloadImg = () => {
  ...
  ctx.filter = `brightness(${brightness}%) saturate(${saturation}%) invert(${inversion}%) blur(${blurW}px)`;
